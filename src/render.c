...
/*   Updated: 2024/08/16 20:05:46 by obenchkr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(t_game *g, int x, float wall_height, t_vec2 ray)
{
	...
	texture_x = (int)(fmodf(ray.x + ray.y, 1.0f) * g->texture.width);
	...
	while (i < end_y)
	{
		plot_pixel(&g->renderer.image, x, i, get_pixel_color(&g->texture.image,
				(int)texture_x, (int)(((i - y) / (2.0f * wall_height))
					* g->texture.height) % g->texture.height));
		i++;
	}
}
